...
export default function Header() {
  ...
  return (
    <header
      className={cn(
        "px-10 top-0 lg:pt-0 right-0 left-0 z-30 fixed flex justify-between items-center mx-auto max-w-[1360px] lg:h-20 h-25 transition-all duration-200",
        isScrolled && "backdrop-blur-xs"
      )}
    >
      <Link href="/" className="flex items-center" onClick={handleLogoClick}>
        <Image src="/logo.svg" alt="Transatron Logo" width={206} height={40} />
      </Link>
      ...
      <div className="hidden lg:flex flex-row gap-4">
        <Button
          variant="default"
          className="hover:bg-transparent px-4! py-3.5! border border-white hover:border-transparent rounded-full h-12 text-lg leading-5 bg-accent-dark"
          onClick={() => {}}
        >
          Request a quote
        </Button>
